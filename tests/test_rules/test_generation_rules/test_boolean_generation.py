...
def test_random_bool_follows_distribution(spark, test_seed):
    ...
    most_true_df = spark.range(5000).select(most_true_rule.generate_column().alias("test_bool"))
    most_true_row = most_true_df.select(
        F.count_if(F.col('test_bool') == True).alias("true_count")).first()
    ...
    all_false_df = spark.range(5000).select(all_false_rule.generate_column().alias("test_bool"))
