...
def test_random_date_bounds_and_type(spark):
    rule = get_rule("random_date", min_date="2020-01-01", max_date="2020-01-10")
    df = spark.range(5000).select(rule.generate_column().alias("test_date"))

    assert isinstance(df.schema["test_date"].dataType, T.DateType)

    row = df.select(F.min("test_date").alias("min_value"), F.max("test_date").alias("max_value")).first()
...
def test_random_int_default_bounds_and_type(spark):
    rule = get_rule("random_int")  # defaults to max range
    ...
    row = df.select(F.min("test_int").alias("min_value"), F.max("test_int").alias("max_value")).first()
    assert row.min_value >= 0
    assert row.max_value <= 2147483647

def test_random_int_custom_inclusive_bounds(spark):
    rule = get_rule("random_int", min_value=1, max_value=5)
...
def test_random_string_length_and_charset(spark):
    rule = get_rule("random_string", min_length=2, max_length=5, charset="abcde")
    ...
    row = length_df.select(F.min("len").alias("min_value"), F.max("len").alias("max_value")).first()
